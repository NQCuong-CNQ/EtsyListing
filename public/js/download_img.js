...
$('body').prepend(`<button id="loadImgsBtn">Load Imgs</button>`)
$('body').prepend(`<button id="enableContextMenu">Bật Right Click</button>`)
$('#loadImgsBtn').on('click', function () {
    document.querySelectorAll('.download-img-tool').forEach(e => e.remove())
    console.log('sdfs')
    let imgs = document.images
    let arrImgs = []
    for (let i = 0; i < imgs.length; i++) {
        if (imgs[i].naturalWidth > 250 && imgs[i].naturalHeight > 250) {
            console.log(imgs[i].currentSrc)
            arrImgs.push(imgs[i].currentSrc)
        }
    }
    for (let i = 0; i < arrImgs.length; i++) {
        $('body').prepend(`<img class="download-img-tool" src="${arrImgs[i]}" width="200" height="200">`)
    }
})

$('#enableContextMenu').on('click', function () {
